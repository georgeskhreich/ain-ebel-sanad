...
const appId = 'ain-ebel-sanad-v1'; // معرّف التطبيق الفريد لبلدية عين إبل

// ==========================================
// البيانات النموذجية الأولية لرفعها عند أول استخدام
// ==========================================
const SEED_SURVEYS = [
  {
...
export default function App() {
  ...
  // رؤية كلمات المرور وتعديلها
  const [visiblePasswords, setVisiblePasswords] = useState({});
  const [editingUser, setEditingUser] = useState(null);

  // إعدادات الاستمارة الميدانية
  ...
  const handleSeedDatabase = async () => {
    ...
    try {
      ...
      for (const item of SEED_SURVEYS) await addDoc(collection(db, 'artifacts', appId, 'public', 'surveys'), item);
      for (const user of SEED_USERS) {
        if (!usersList.some(u => u.username === user.username)) await addDoc(collection(db, 'artifacts', appId, 'public', 'users'), user);
      }
  ...
  const handleFormSubmit = async () => {
    ...
    // تصفير وفتح ملف جديد مباشرة للمهندس
    setWizardStep(1);
